...
<!-- Mirrored from www.w3school.com.cn/aspnet/mvc_publish.asp by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 02 Dec 2015 03:31:27 GMT -->
<head>
<meta charset="gb2312" />
...
<li class="currentLink"><a href="mvc_publish.asp" title="MVC 发布">MVC 发布</a></li>
...
<h1>ASP.NET MVC - 发布网站</h1>


<div id="tpn">
<ul class="prenext">
<li class="pre"><a href="mvc_htmlhelpers.html" title="MVC HTML 助手">MVC HTML 助手</a></li>
<li class="next"><a href="mvc_reference.html" title="MVC 参考手册">MVC 参考手册</a></li>
</ul>
</div>


<div id="intro">
<p>学习如何在不使用 Visual Web Developer 的情况下发布 MVC 应用程序。</p>
</div>


<div>
<h2>在不使用  Visual Web Developer 的情况下发布您的应用程序</h2>

<p>通过在 WebMatrix、Visual Web Developer 或 Visual Studio 中使用发布命令，可以将 ASP.NET MVC 应用程序发布到远程服务器。</p>

<p>该功能会复制所有应用程序文件、控制器、模型、图像以及所有必需的 DLL 文件，这些文件可能用于 MVC、Web Pages、Razor、Helpers、SQL Server Compact（如果使用了数据库）。</p>

<p>有时您不希望使用这个选项。也许您的主机提供商只支持 FTP？也许您的网站基于经典 ASP？也许您希望自己来复制文件？也许您使用的是其他发布软件？</p>

<p class="marked">您会遇到问题吗？是的，会的。但是我们能解决它。</p>

<p>要执行网站复制，您必须了解如何引用正确的文件，复制哪些 DLL  文件，把它们存放到何处。</p>
...
<div>
<h2>1. 使用最新版本的 ASP.NET</h2>

<p>在继续之前，请确保您的主机运行最新版本的 ASP.NET (4.0)。</p>
</div>
...
<p>从开发机上把您的网站（所有文件夹和内容）复制到远程主机（服务器）上的应用程序文件夹。</p>

<p>如果 <b>App_Data</b> 文件夹中包含测试数据，请不要复制这个 App_Data 文件夹。</p>
</div>
...
<p>在远程服务器上的应用程序根目录中创建 bin 文件夹。（如果您已安装帮助器，则 bin 文件夹已经存在）</p>

<p>从您的文件夹中复制以下所有文件：</p>

<p><b>C:\Program Files (x86)\Microsoft ASP.NET\ASP.NET Web Pages\v1.0\Assemblies</b></p>

<p><b>C:\Program Files (x86)\Microsoft ASP.NET\ASP.NET MVC 3\Assemblies</b></p>

<p>到远程服务器上的 bin 文件夹。</p>
</div>


<div>
<h2>4. 复制 SQL Server Compact DLL 文件</h2>

<p>如果您的应用程序使用了 SQL Server Compact 数据库（App_Data 文件夹中的 .sdf 文件），那么您必须复制 SQL Server Compact DLL 文件：</p>

<p>从您的文件夹复制以下所有文件：</p>

<p><b>C:\Program Files (x86)\Microsoft SQL Server Compact Edition\v4.0\Private</b></p>

<p>到远程服务器上的 bin 文件夹。</p>

<p>创建或编辑应用程序中的 Web.config 文件：</p>
...
<div>
<h2>5. 复制 SQL Server Compact 数据</h2>

<p>您的 App_Data 文件夹中有没有包含测试数据的 .sdf 文件？</p>

<p>您是否希望将测试数据发布到远程服务器？</p>

<p>大多数时候是不希望。</p>

<p>如果您不得不复制 SQL 数据文件（sdf 文件），那么您应该删除数据库中的所有数据，然后把这个空的 .sdf 文件从开发机复制到服务器。</p>
