...
<!-- Mirrored from www.w3school.com.cn/aspnet/mvc_views.asp by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 02 Dec 2015 03:31:27 GMT -->
<head>
<meta charset="gb2312" />
...
<li class="currentLink"><a href="mvc_views.asp" title="MVC 视图">MVC 视图</a></li>
...
<h1>ASP.NET MVC - 视图</h1>


<div id="tpn">
<ul class="prenext">
<li class="pre"><a href="mvc_controllers.html" title="MVC 控制器">MVC 控制器</a></li>
<li class="next"><a href="mvc_database.html" title="MVC 数据库">MVC 数据库</a></li>
</ul>
</div>
...
<p class="note"><span>部分 5：</span>添加用于显示应用程序的视图。</p>
</div>
...
<p><em>Views</em> 文件夹存储的是与应用程序显示（用户界面）相关的文件（HTML 文件）。根据语言的不同，这些文件的扩展名可能是 html、asp、aspx、cshtml 以及 vbhtml。</p>

<p>Views 文件夹包含每个控制器对应的一个文件夹。</p>

<p>Visual Web Developer 已创建了一个 Account 文件夹、一个 Home 文件夹、一个 Shared 文件夹（在 Views 文件夹内）。</p>

<p>Account 文件夹包含用于注册并登录用户帐户的页面。</p>

<p>Home 文件夹用于存储诸如首页和关于页之类的应用程序页面。</p>

<p>Shared 文件夹用于存储控制器间分享的视图（模板页和布局页）。</p>
</div>
...
<p>能够在 Views 文件夹中找到以下 HTML 文件类型：</p>

<table class="dataintable">
<tr>
<th style="width:40%;">文件类型</th>
<th>扩展名</th>
</tr>
...
<p>文件 Index.cshtml 表示应用程序的首页。它是应用程序的默认文件（首页文件）。</p>
...
<p>文件 About.cshtml 表示应用程序的关于页面。</p>

<p>在文件中写入以下内容：</p>

<pre>@{ViewBag.Title = &quot;About Us&quot;;}
...
<p>选择调试，从调试菜单中启动调试（或者按 F5）。</p>

<p>您的应用程序将类似这样：</p>


<p>请点击“首页”和“关于”，看看它是如何运行的。</p>
</div>
...
<p>祝贺您。您已经创建好了第一个 MVC 应用程序。</p>

<p class="note"><span>注释：</span>“电影”选项卡仍然无法点击。我们将在本教程稍后的章节中为“电影”选项卡添加代码。</p>
</div>
