...
<!-- Mirrored from www.w3school.com.cn/aspnet/webpages_ref_database.asp by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 02 Dec 2015 05:12:40 GMT -->
<head>
<meta charset="gb2312" />
...
<title>ASP.NET Web Pages - Database 对象</title>
...
<li class="currentLink"><a href="webpages_ref_database.asp" title="WebPages 数据库">WebPages 数据库</a></li>
...
<h1>ASP.NET Web Pages - Database 对象</h1>


<div id="tpn">
<ul class="prenext">
<li class="pre"><a href="webpages_ref_websecurity.asp" title="WebPages 安全">WebPages 安全</a></li>
<li class="next"><a href="webpages_ref_webmail.html" title="WebPages WebMail">WebPages WebMail</a></li>
</ul>
</div>


<div>
<h2>ASP.NET Database 对象参考手册</h2>
...
<dt>Dispose()</dt>
<dd>释放由 Database 实例使用的所有资源。</dd>

<dt>Database.Execute(SQLstatement [, parameters]</dt>
<dd>执行 SQL 语句并返回受影响的记录。</dd>

<dt>Database.GetLastInsertId()</dt>
<dd>返回最近一次插入的行的标识列。</dd>

<dt>Database.Open(filename)<br />
Database.Open(connectionStringName)</dt>
<dd>使用指定的文件名或指定的连接字符串打开到数据库的连接。</dd>

<dt>Database.OpenConnectionString(connectionString)</dt>
<dd>使用指定的连接字符串打开到数据库的连接。</dd>

<dt>Database.Query(SQLstatement[, parameters])</dt>
<dd>执行 SQL 查询，该查询将返回行的列表作为结果。</dd>

<dt>Database.QuerySingle(SQLstatement [, parameters])</dt>
<dd>执行 SQL 查询，该查询将返回一行作为结果。</dd>

<dt>Database.QueryValue(SQLstatement [, parameters])</dt>
<dd>执行 SQL 查询，该查询将返回一个标量值作为结果。</dd>
</dl>
</div>
