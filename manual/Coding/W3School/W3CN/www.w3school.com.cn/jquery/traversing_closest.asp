...
<!-- Mirrored from www.w3school.com.cn/jquery/traversing_closest.asp by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 02 Dec 2015 05:49:33 GMT -->
<head>
<meta charset="gbk" />
...
<title>jQuery 遍历 - closest() 方法</title>
</head>
...
<h1>jQuery 遍历 - closest() 方法</h1>

<div class="backtoreference">
<p><a href="jquery_ref_traversing.html" title="jQuery 参考手册 - 遍历">jQuery 遍历参考手册</a></p>
</div>
...
<p>本例演示如何通过 closest() 完成事件委托。当被最接近的列表元素或其子后代元素被点击时，会切换黄色背景：</p>

<pre>
$( document ).bind(&quot;click&quot;, function( e ) {
    $( e.target )<code>.closest(&quot;li&quot;)</code>.toggleClass(&quot;hilight&quot;);
  });
</pre>

<p class="tiy"><a target="_blank" href="../tiy/t91cf.asp?f=jquery_traversing_closest">亲自试一试</a></p>
</div>
...
<p>closest() 方法获得匹配选择器的第一个祖先元素，从当前元素开始沿 DOM 树向上。</p>

<h3>语法</h3>

<pre>.closest(<i>selector</i>)</pre>
...
<p>如果给定表示 DOM 元素集合的 jQuery 对象，.closest() 方法允许我们检索 DOM 树中的这些元素以及它们的祖先元素，并用匹配元素构造新的 jQuery 对象。<a href="traversing_parents.html">.parents()</a> 和 .closest() 方法类似，它们都沿 DOM 树向上遍历。两者之间的差异尽管微妙，却很重要：</p>

<table class="dataintable">
<tr>
<th style="width:50%;">.closest()</th>
<th>.parents()</th>
</tr>
...
<tr>
<td>沿 DOM 树向上遍历，直到找到已应用选择器的一个匹配为止。</td>
<td>沿 DOM 树向上遍历，直到文档的根元素为止，将每个祖先元素添加到一个临时的集合；如果应用了选择器，则会基于该选择器对这个集合进行筛选。</td>
</tr>

<tr>
<td>返回包含零个或一个元素的 jQuery 对象</td>
<td>返回包含零个、一个或多个元素的 jQuery 对象</td>
</tr>
</table>
...
<p>假设我们执行一个从项目 A 开始的对 &lt;ul&gt; 元素的搜索：</p>

<pre>$('li.item-a').closest('ul').css('background-color', 'red');</pre>

<p class="tiy"><a target="_blank" href="../tiy/t5ab7.html?f=jquery_traversing_closest_2">亲自试一试</a></p>

<p>这会改变 level-2 &lt;ul&gt; 的颜色，这是因为当向上遍历 DOM 树时会第一个遇到该元素。</p>

<h3>例子 2</h3>

<p>假设我们搜索的是 &lt;li&gt; 元素：</p>

<pre>$('li.item-a').closest('li').css('background-color', 'red');</pre>

<p class="tiy"><a target="_blank" href="../tiy/t9343.html?f=jquery_traversing_closest_3">亲自试一试</a></p>

<p>这会改变列表项目 A 的颜色。在向上遍历 DOM 树之前，.closest() 方法会从 li 元素本身开始搜索，直到选择器匹配项目 A 为止。</p>

<h3>例子 3</h3>

<p>我们可以传递 DOM 元素作为 context，在其中搜索最接近的元素。</p>
...
<p class="tiy"><a target="_blank" href="../tiy/t154d.asp?f=jquery_traversing_closest_4">亲自试一试</a></p>

<p>以上代码会改变 level-2 &lt;ul&gt; 的颜色，因为它既是列表项 A 的第一个 &lt;ul&gt; 祖先，同时也是列表项 II 的后代。它不会改变 level-1 &lt;ul&gt; 的颜色，因为它不是 list item II 的后代。</p>
...
<!-- Mirrored from www.w3school.com.cn/jquery/traversing_closest.asp by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 02 Dec 2015 05:50:16 GMT -->
</html>