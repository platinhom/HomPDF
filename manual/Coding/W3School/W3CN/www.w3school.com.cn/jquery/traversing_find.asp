...
<!-- Mirrored from www.w3school.com.cn/jquery/traversing_find.asp by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 02 Dec 2015 05:51:04 GMT -->
<head>
<meta charset="gbk" />
...
<p>搜索所有段落中的后代 span 元素，并将其颜色设置为红色：</p>

<pre>
$(&quot;p&quot;)<code>.find(&quot;span&quot;)</code>.css('color','red');
</pre>

<p class="tiy"><a target="_blank" href="../tiy/t642d.html?f=jquery_traversing_find">亲自试一试</a></p>
</div>
...
<p>find() 方法获得当前元素集合中每个元素的后代，通过选择器、jQuery 对象或元素来筛选。</p>

<h3>语法</h3>

<pre>.find(<i>selector</i>)</pre>
...
<tr>
<td><i>selector</i></td>
<td>字符串值，包含供匹配当前元素集合的选择器表达式。</td>
</tr>
</table>

<h3>详细说明</h3>

<p>如果给定一个表示 DOM 元素集合的 jQuery 对象，.find() 方法允许我们在 DOM 树中搜索这些元素的后代，并用匹配元素来构造一个新的 jQuery 对象。.find() 与 .children() 方法类似，不同的是后者仅沿着 DOM 树向下遍历单一层级。</p>

<p>.find() 方法第一个明显特征是，其接受的选择器表达式与我们向 $() 函数传递的表达式的类型相同。将通过测试这些元素是否匹配该表达式来对元素进行过滤。</p>
...
<p>我们将从列表 II 开始来查找其中的列表项：</p>

<pre>$('li.item-ii').find('li').css('background-color', 'red');</pre>

<p class="tiy"><a target="_blank" href="../tiy/t8eba.html?f=jquery_traversing_find_2">亲自试一试</a></p>

<p>这次调研的结果是，项目 A、B、1、2、3 以及 C 均被设置了红色背景。即使项目 II 匹配选择器表达式，它也不会被包含在结果中；只会对后代进行匹配。</p>

<p>与其他的树遍历方法不同，选择器表达式对于 .find() 是必需的参数。如果我们需要实现对所有后代元素的取回，可以传递通配选择器 '*'。</p>

<p>选择器 context 是由 .find() 方法实现的；因此，$('li.item-ii').find('li') 等价于 $('li', 'li.item-ii')。</p>

<p>对于 jQuery 1.6，我们还可以使用给定的 jQuery 集合或元素来进行筛选。还是上面的嵌套列表，我们首先这样写：</p>

<pre>var $allListElements = $('li');</pre>

<p>然后将这个 jQuery 对象传递给 find 方法：</p>

<pre>$('li.item-ii')<code>.find( $allListElements )</code>;</pre>

<p class="tiy"><a target="_blank" href="../tiy/tcfcb.html?f=jquery_traversing_find_3">亲自试一试</a></p>

<p>上面的代码会返回一个 jQuery 集合，其中包含属于列表 II 后代的列表元素。</p>

<p>类似地，也可以传递一个元素：</p>

<pre>
var item1 = $('li.item-1')[0];
$('li.item-ii')<code>.find( item1 )</code>.css('background-color', 'red');
</pre>

<p class="tiy"><a target="_blank" href="../tiy/t73b7.html?f=jquery_traversing_find_4">亲自试一试</a></p>

<p>这次调用的结果是项目 1 被设置为红色背景。</p>
</div>
...
<!-- Mirrored from www.w3school.com.cn/jquery/traversing_find.asp by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 02 Dec 2015 05:51:07 GMT -->
</html>