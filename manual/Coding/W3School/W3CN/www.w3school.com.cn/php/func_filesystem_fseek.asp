...
<!-- Mirrored from www.w3school.com.cn/php/func_filesystem_fseek.asp by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 02 Dec 2015 07:12:43 GMT -->
<head>
<meta charset="gbk" />
...
<p><a href="php_ref_filesystem.html" title="PHP Filesystem 函数">PHP Filesystem 函数</a></p>
</div>

<div>
<h2>定义和用法</h2>

<p>fseek() 函数在打开的文件中定位。</p>

<p>该函数把文件指针从当前位置向前或向后移动到新的位置，新位置从文件头开始以字节数度量。</p>

<p>成功则返回 0；否则返回 -1。注意，移动到 EOF 之后的位置不会产生错误。</p>

<h3>语法</h3>
<pre>fseek(file,offset,whence)</pre>
...
<table class="dataintable">
  ...
  <tr>
    <td>file</td>
    <td>必需。规定要在其中定位的文件。</td>
  </tr>
  <tr>
    <td>offset</td>
    <td>必需。规定新的位置（从文件头开始以字节数度量）。</td>
  </tr>
  <tr>
    <td>whence</td>
    <td>
	...
	<ul class="listintable">
		<li>SEEK_SET - 设定位置等于 <i>offset</i> 字节。默认。</li>
		<li>SEEK_CUR - 设定位置为当前位置加上 <i>offset</i>。</li>
		<li>SEEK_END - 设定位置为文件末尾加上 <i>offset</i> （要移动到文件尾之前的位置，<i>offset</i> 必须是一个负值）。</li>
	</ul>
	</td>
  </tr>
</table>
...
<p><i>whence</i> 参数是 PHP 4.0.0 之后增加的。</p>
...
<p class="tip"><span>提示：</span>通过使用 <a href="func_filesystem_ftell.html" title="PHP ftell() 函数">ftell()</a> 来找到当前位置。</p>
</div>
