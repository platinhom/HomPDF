...
<!-- Mirrored from www.w3school.com.cn/php/func_array_intersect_uassoc.asp by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 02 Dec 2015 05:53:30 GMT -->
<head>
<meta charset="gbk" />
...
<title>PHP array_intersect_uassoc() 函数</title>
...
<h1>PHP array_intersect_uassoc() 函数</h1>
...
<p>array_intersect_uassoc() 函数使用用户自定义的回调函数计算数组的交集，用回调函数比较索引。</p>

<p>array_intersect_uassoc() 返回一个数组，该数组包含了所有在 <i>array1</i> 中也同时出现在所有其它参数数组中的值。返回的数组中键名保持不变。</p>

<p>注意，与 <a href="func_array_intersect.html" title="PHP array_intersect() 函数">array_intersect()</a> 不同的是除了比较键值，还要比较键名。</p>

<p>此比较是通过用户提供的回调函数来进行的。该函数带有两个参数，即两个要进行对比的键名。如果第一个参数小于第二个参数，则函数要返回一个负数，如果两个参数相等，则要返回 0，如果第一个参数大于第二个参数，则返回一个正数。</p>

<h3>语法</h3>
<pre>array_intersect_uassoc(array1,array2,array3...,function)</pre>
