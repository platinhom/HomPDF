...
<!-- Mirrored from www.w3school.com.cn/php/func_array_next.asp by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 02 Dec 2015 05:53:30 GMT -->
<head>
<meta charset="gbk" />
...
<p>next() 函数把指向当前元素的指针移动到下一个元素的位置，并返回当前元素的值。</p>

<p>如果内部指针已经超过数组的最后一个元素，函数返回 false。</p>
...
<p>next() 和 <a href="func_array_current.html" title="PHP current() 函数">current()</a> 的行为类似，只有一点区别，在返回值之前将内部指针向前移动一位。这意味着它返回的是下一个数组单元的值并将数组指针向前移动了一位。如果移动指针的结果超出了数组单元的末端，则 next() 返回 FALSE。</p>

<p class="important"><span>注意：</span>如果数组包含空的单元，或者单元的值是 0 则该函数碰到这些单元也返回 FALSE。要正确遍历可能含有空单元或者单元值为 0 的数组，请参见 <a href="func_array_each.html" title="PHP each() 函数">each() 函数</a>。</p>
