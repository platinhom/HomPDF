...
<!-- Mirrored from www.w3school.com.cn/php/func_xml_set_notation_decl_handler.asp by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 02 Dec 2015 07:12:43 GMT -->
<head>
<meta charset="gbk" />
...
<title>PHP xml_set_notation_decl_handler() 函数</title>
...
<h1>PHP xml_set_notation_decl_handler() 函数</h1>

<div class="backtoreference">
<p><a href="php_ref_xml.asp" title="PHP XML Parser 函数">PHP XML Parser 函数</a></p>
</div>

<div>
<h2>定义和用法</h2>

<p>xml_set_notation_decl_handler() 函数规定当解析器在 XML 文档中找到符号声明时被调用的函数。</p>

<p>如果处理器被成功的建立，该函数将返回 true；否则返回 false。</p>

<p class="note"><span>注释：</span>“符号声明”，英文为 notation declaration，也有部分文献译为“注释声明”。</p>


<h3>语法</h3>
<pre>xml_set_notation_decl_handler(parser,handler)</pre>
...
<table class="dataintable"> 
  ...
  <tr>
    <td>parser</td>
    <td>必需。规定要使用的 XML 解析器。</td>
  </tr>
	<tr>
    <td>handler</td>
    <td>必需。规定当解析器找到符号声明时被调用的函数。</td>
  </tr>
</table>

<p>由 <i>handler</i> 参数规定的函数必须有六个参数：</p>
...
<table class="dataintable"> 
  ...
  <tr>
    <td>parser</td>
    <td>必需。规定一个变量，包含调用处理器的 XML 解析器。</td>
  </tr>
  <tr>
    <td>name</td>
    <td>必需。规定包含实体名称的变量。</td>
  </tr>
  <tr>
    <td>base</td>
    <td>
	<p>必需。规定一个变量，包含解析实体的系统标识符（system_id ）的基础。</p>
	<p>当前该参数通常都被设置为空字符串。</p>
	</td>
  </tr>
  <tr>
    <td>system_id</td>
    <td>必需。规定包含实体的系统标识符的变量。</td>
  </tr>
  <tr>
    <td>public_id</td>
    <td>必需。规定包含实体的公共标识符的变量。</td>
  </tr>
  <tr>
    <td>notation</td>
    <td>必需。规定一个变量，包含标识实体数据类型的符号。</td>
  </tr>
</table>

<h3>说明</h3>

<p><i>handler</i> 参数也可以是一个数组，其中包含对象引用和方法名。</p>
...
<code>xml_set_notation_decl_handler($parser, &quot;not_decl_handler&quot;);</code>
$fp=fopen(&quot;test.xml&quot;,&quot;r&quot;);
