...
<!-- Mirrored from www.w3school.com.cn/php/func_math_expm1.asp by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 02 Dec 2015 05:53:31 GMT -->
<head>
<meta charset="gbk" />
...
<p>expm1() 函数返回 exp(x) - 1，甚至当 number 的值接近零也能计算出准确结果。</p>
...
<p>expm1() 返回 'exp(x) - 1'，甚至当 <i>x</i> 的值接近零也能计算出准确结果。但是当两个数值趋近于相等的时候， 'exp (x) - 1' 就会变得不太准确。</p>
...
<p class="important"><span>警告：</span>本函数是实验性的。本函数的行为，包括函数名称以及其它任何关于本函数的文档可能会在没有通知的情况下随 PHP 以后的发布而改变。使用本函数风险自担。</p>

<p class="tip"><span>提示：</span>用 'e' 作为自然对数的底 2.718282。</p>

<p class="note"><span>注释：</span>本函数未在 Windows 平台下实现。</p>
</div>
