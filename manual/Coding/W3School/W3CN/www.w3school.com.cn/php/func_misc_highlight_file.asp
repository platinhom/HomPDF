...
<!-- Mirrored from www.w3school.com.cn/php/func_misc_highlight_file.asp by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 02 Dec 2015 05:53:33 GMT -->
<head>
<meta charset="gbk" />
...
<title>PHP highlight_file() 函数</title>
...
<h1>PHP highlight_file() 函数</h1>

<div class="backtoreference">
<p><a href="php_ref_misc.html" title="PHP 杂项函数">PHP 杂项函数</a></p>
</div>
...
<h3>语法</h3>
<pre>highlight_file(filename,return)</pre>
...
<table class="dataintable"> 
  ...
  <tr>
    <td>filename</td>
    <td>必需。要进行高亮处理的 PHP 文件的路径。</td>
  </tr>
  <tr>
    <td>return</td>
    <td>可选。如果设置 true，则本函数返回高亮处理的代码。</td>
  </tr>
</table>

<h3>说明</h3>

<p>本函数通过使用 PHP 语法高亮程序中定义的颜色，输出或返回包含在 <i>filename</i> 中的代码的语法高亮版本。</p>

<p>许多服务器被配置为对带有 <i>phps</i> 后缀的文件进行自动高亮处理。例如，在查看 example.phps 时，将显示该文件被语法高亮显示的源代码。要启用该功能，请把下面这一行添加到 <i>httpd.conf</i>：</p>

<pre>AddType application/x-httpd-php-source .phps</pre>

<h3>返回值</h3>

<p>如果 <i>return</i> 参数被设置为 true，那么该函数会返回被高亮处理的代码，而不是输出它们。否则，若成功，则返回 true，失败则返回 false。</p>
...
<p class="important"><span>警告：</span>需要注意的是，在使用 highlight_file() 函数时，请不要因为疏忽而泄露诸如密码或其他类型的敏感信息，否则会出现潜在的安全风险。</p>
</div>
...
highlight_file</span><span style="COLOR: #007700">(</span><span style="COLOR: #dd0000">&quot;test.php&quot;</span><span style="COLOR: #007700">); </span>
